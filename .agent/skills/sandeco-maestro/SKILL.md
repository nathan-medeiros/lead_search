...
# Skill: Maestro (Orquestração Multi-Agente)
...
- `.agents/skills/sandeco-maestro/.antigravity/equipe/registro_atividades.json` → Registro mestre de atividades, estados e pré-requisitos.
- `.agents/skills/sandeco-maestro/.antigravity/equipe/caixa_entrada/` → Comunicações individuais entre agentes (.msg).
- `.agents/skills/sandeco-maestro/.antigravity/equipe/aviso_geral.msg` → Comunicados globais para todo o esquadrão.
- `.agents/skills/sandeco-maestro/.antigravity/equipe/travas/` → Semáforos para impedir edição simultânea de arquivos.

# IMPORTANTE
1. Nunca crie uma nova pasta `.antigravity` na raiz do projeto.
Utilize a pasta `.antigravity` que fica dentro do diretório desta skill (`.agents/skills/sandeco-maestro/.antigravity/`).
...
1. **Condutor (Maestro)**: O líder do time. Decompõe o problema, distribui responsabilidades e valida planos de ação.
...
Antes de efetuar alterações relevantes, cada agente deve submeter um **Plano de Ação** à caixa de entrada do Maestro.

- O agente permanece em modo `SOMENTE_LEITURA` ou `PLANEJAMENTO` até que o Maestro responda com um comunicado de `APROVADO`.

### 2. Comunicação e Difusão (Broadcast)

- **Comunicação Direta**: Coordenação 1-a-1 entre executores.
- **Comunicado Geral**: Maestro pode escrever em `aviso_geral.msg` para transmitir novas orientações a todo o esquadrão simultaneamente.
...
- NUNCA editar um arquivo se existir um .lock ativo em `.agents/skills/sandeco-maestro/.antigravity/equipe/travas/`.
- Ao finalizar uma atividade, o agente deve liberar suas "travas" e notificar o Maestro.
